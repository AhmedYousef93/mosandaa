        ...
        <div class="col-12">
            ...
            <div class="breadcrumb-wrapper">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item"><a href="">@lang('admin.areas')</a>
                    </li>
                </ol>
            </div>
        </div>
...
<section>
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header border-bottom">
                    <h4 class="card-title"> @lang('admin.areas')</h4>
                    @if(Auth::guard('admin')->user()->hasPermission('areas-create'))
                    <button class="dt-button create-new btn btn-gradient-success d-inline-flex" type="button"
                        data-toggle="modal" data-target="#modal-add-area">
                        <span style="font-size: 15px;">+ @lang('admin.addareas')</span>
                    </button>
                    @else
                ...
                <div class="card-datatable">
                    {{ $dataTable->table([
                    'class'=> 'datatables-ajax table table-bordered area-table'
                    ],true) }}
    ...
    @if(Auth::guard('admin')->user()->hasPermission('areas-create'))
    <div class="modal modal-slide-in fade" id="modal-add-area">
        <div class="modal-dialog sidebar-sm">
            <form id="addarea" method="POST" class="add-new-record modal-content pt-0" enctype="multipart/form-data"
                ...
                <div class="modal-header mb-1">
                    <h5 class="modal-title" id="exampleModalLabel">@lang('admin.addareas')</h5>
                </div>
    ...
    @if(Auth::guard('admin')->user()->hasPermission('areas-update'))
    <div class="modal modal-slide-in fade" id="modal-edit-area">
        <div class="modal-dialog sidebar-sm">
            <form id="editarea" method="post" class="add-new-record modal-content pt-0" enctype="multipart/form-data"
                data-parsley-validate>
                @csrf
                @method('put')

                <input type="hidden" name="areaId" id="area_id" class="form-control">

                <button type="button" class="close" data-dismiss="modal" aria-label="Close">×</button>
                <div class="modal-header mb-1">
                    <h5 class="modal-title" id="exampleModalLabel">@lang('admin.editareas')</h5>
                </div>
                <div class="modal-body flex-grow-1">
...
<script>
    ...
    $('body').on('submit', '#addarea', function (e) {
        e.preventDefault();
        $.ajax({
    ...
    $('body').on('submit', '#editarea', function (e) {
        e.preventDefault();
        let id = $('#area_id').val();
        var url = '{{ route('areas.update', ':id') }}';
